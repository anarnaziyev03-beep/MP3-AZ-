...
def handle_message(message):
  ...
  try:
    ...
    for i, entry in enumerate(entries, 1):
      title = entry.get("title", "Adsız")
      duration = entry.get("duration_string", "N/A")
      result_text += f"{i}. {title} ({duration})\n"
      markup.add(
          InlineKeyboardButton(f"📥 {i}-ci mahnıyı yüklə", callback_data=f"d1_{i-1}")
      )
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("d1_"))
def callback_inline(call):
  ...
  if chat_id in search_results and index < len(search_results[chat_id]):
    ...
    bot.answer_callback_query(call.id, "Məlumat tapılmadı, yenidən axtarın.")
